        ...
        success: function (res) {
            ...
            $.each(res.daySummaryExtendDOS, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    ...
                    "<td name='column' id='"+obj["daySummaryId"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
                    "<td name='column'>" + obj["day_summary_time"] + "</td>" +
                    "<td name='column'>" + obj["day_summary_entry_time"] + "</td>" +
                    "<td><a href='#' id='"+obj["daySummaryId"]+"\' class='month_a'>查看</a></td>" +
                    "<td id='"+obj["day_summary_code"]+"abc"+"\'>默认</td>"+
                    "<td><input type=\"button\" value=\"审核\" name='"+obj["daySummaryId"]+"\' class=\"btn btn-primary btn-lg active btn btn-primary btn-sm\" id='btn' onclick='sss(this.name)'/></td>" +
                    "</tr>"
                );
                //判断日志状态
                if(obj["day_summary_state"]==1){
                    document.getElementById(obj["day_summary_code"]+"abc").innerHTML = "以保存";
                    ...
                    document.getElementById(obj["day_summary_code"]+"abc").innerHTML = "请重写";
        ...
        $.ajax({
            ...
            success:function (res) {
                if(res.code==null) {
                    ...
                    // alert(trSeq)
                    //日总结
                    oneP_sum_day.value = res.daySummaryExtendDO.day_summary_content;
                    oneP_sumend_day.value = res.daySummaryExtendDO.day_summary_entry_time;
                    //日计划
                    oneP_plan_day.value=res.daySummaryExtendDO.dayPlanDO.day_plan_content;
                    oneP_end_day.value=res.daySummaryExtendDO.dayPlanDO.day_plan_entry_time;
                    //周计划
                    oneP_plan_week.value=res.daySummaryExtendDO.weekPlanDO.week_plan_content;
                    oneP_end_week.value=res.daySummaryExtendDO.weekPlanDO.week_plan_entry_time;
                    //月计划
                    oneP_plan.value=res.daySummaryExtendDO.monthlyPlan.monthly_plan_content;
                    oneP_end.value=res.daySummaryExtendDO.monthlyPlan.monthly_plan_entry_time;
                }
            },
            error: function(){
                alert("ajax执行失败！");
