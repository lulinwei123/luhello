function init_sidebar() {
	...
	$SIDEBAR_MENU.find("a").on("click", function(b) {
		var c = $(this).parent();
		c.is(".active") ? (c.removeClass("active active-sm"), $("ul:first", c).stop().slideUp(function() {
			a()
		})) : (c.parent().is(".child_menu") ? $BODY.is(".nav-sm") && ($SIDEBAR_MENU.find("li").removeClass("active active-sm"), $SIDEBAR_MENU.find("li ul").stop().slideUp()) : ($SIDEBAR_MENU.find("li").removeClass("active active-sm"), $SIDEBAR_MENU.find("li ul").stop().slideUp()), c.addClass("active"), $("ul:first", c).stop().slideDown(function() {
			a()
		}))
...
function sql(){
	//获取id
	var sqlh = document.getElementById("mysql").value;
	//后缀名验证
    var regsql = /\.(sql)$/;
	if(!regsql.test(sqlh)) {
		document.getElementById("span").innerHTML = "*必须包含指定后缀名 .sql!";
		return false;
	}
	document.getElementById("span").innerHTML = "OK 下一步!";
	return true;
}
...
$(function(){
	...
	//点击取消模态框
	$(document).on("click" , ".back_false" , function(){
		$(".addfade").fadeOut(300);
	});
	
	//点击保存
	$(document).on("click" , ".back_true" , function(){
		$(".addfade").fadeOut(300);
	});
	...
	$(document).on("click" , ".rest_true" , function(){
		$(".movefade").fadeOut(300);
	})
	...
	$(document).on("click" , ".back_true" , function(){
	   ...
	   var flag = false;
	   if(flag){
	   	$("#viewmodal").find(".modal-header i").addClass('fa fa-check fa-hfl');
	   	$("#viewmodal").find(".modal-body").html("备份成功");
	   	$(".viewfade").fadeOut(2000);
	   }else{
	   	$("#viewmodal").find(".modal-header i").addClass('fa fa-times fa-hfl');
	   		$("#viewmodal").find(".modal-body").html("备份失败").css("color" , "red");
	   		$(".viewfade").fadeOut(2000);
	   };
