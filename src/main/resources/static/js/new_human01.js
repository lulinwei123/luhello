        ...
        success: function (res) {
            ...
            $.each(res.monthlySummaryExtendDOS, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    ...
                    "<td name='column' id='"+obj["monthlySummaryId"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
                    "<td name='column'>" + obj["monthly_summary_time"] + "</td>" +
                    "<td name='column'>" + obj["monthly_summary_entry_time"] + "</td>" +
                    "<td><a href='#' id='"+obj["monthlySummaryId"]+"\' class='month_a'>查看</a></td>" +
                    "<td id='"+obj["monthly_summary_code"]+"abc"+"\'>默认</td>"+
                    "<td><input type=\"button\" value=\"审核\" name='"+obj["monthlySummaryId"]+"\' class=\"btn btn-primary btn-lg active btn btn-primary btn-sm\" id='btn' onclick='sss(this.name)'/></td>" +
                ...
                if(obj["monthly_summary_state"]==1){
                    document.getElementById(obj["monthly_summary_code"]+"abc").innerHTML = "以保存";
                    ...
                    document.getElementById(obj["monthly_summary_code"]+"abc").innerHTML = "请重写";
        ...
        $.ajax({
            ...
            success:function (res) {
                if (res.code==null){
                    ...
                    // alert(trSeq)
                    oneP_plan.value = res.monthlySummaryExtendDO.monthly_summary_content;
                    oneP_end.value = res.monthlySummaryExtendDO.monthly_summary_entry_time;
                    oneP_Month_Plan.value = res.monthlySummaryExtendDO.monthlyPlan.monthly_plan_content;
                }
