        ...
        success: function (res) {
            ...
            $.each(res.dayPlanExtendDOS, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    ...
                    "<td name='column' id='"+obj["dayPlanId"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
                    "<td name='column'>" + obj["day_plan_time"] + "</td>" +
                    "<td name='column'>" + obj["day_plan_entry_time"] + "</td>" +
                    "<td><a href='#' id='"+obj["dayPlanId"]+"\' class='month_a'>查看</a></td>" +
                    "<td id='"+obj["day_plan_code"]+"abc"+"\'>默认</td>"+
                    "<td><input type=\"button\" value=\"审核\" name='"+obj["dayPlanId"]+"\' class=\"btn btn-primary btn-lg active btn btn-primary btn-sm\" id='btn' onclick='sss(this.name)'/></td>" +
                ...
                if(obj["day_plan_state"]==1){
                    document.getElementById(obj["day_plan_code"]+"abc").innerHTML = "以保存";
                }else if (obj["day_plan_state"]==2){
                    document.getElementById(obj["day_plan_code"]+"abc").innerHTML = "待审核";
                }else if (obj["day_plan_state"]==3){
                    document.getElementById(obj["day_plan_code"]+"abc").innerHTML = "请重写";
        ...
        $.ajax({
            ...
            success:function (res) {
                if (res.code==null){
                    ...
                    // alert(trSeq)
                    //日计划
                    oneP_plan_day.value = res.dayPlanExtendDO.day_plan_content;
                    oneP_end_day.value = res.dayPlanExtendDO.day_plan_entry_time;
                    //周计划
                    oneP_plan_week.value=res.dayPlanExtendDO.weekPlanDO.week_plan_content;
                    oneP_end_week.value=res.dayPlanExtendDO.weekPlanDO.week_plan_entry_time;
                    //月计划
                    oneP_plan.value=res.dayPlanExtendDO.monthlyPlan.monthly_plan_content;
                    oneP_end.value=res.dayPlanExtendDO.monthlyPlan.monthly_plan_entry_time;
                }
